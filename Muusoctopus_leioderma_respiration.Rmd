...
Running the linear mixed effects model and ANOVA using type III sum of squares. I am excluding one octopus (2-1) from the analysis for which the day 1 data was lost (raw data file was misplaced or accidentally deleted).
...
convert Figure_3.png Figure_3.jpg
```
...
```{r}
logmass=mean(routine$mass.log[routine$octo!="2-1"]) #mean log of masses that went into the lme
...
# How much do the small octopuses affect the end results?
One reviewer raised the concerns that the highly variability of RMR in small octopuses might skew the results. Here, I rum the lme again without those two octopuses and see how much they influence the final results.

```{r low excluded linear mixed effects model}
low.lme=lme(rmr.log~mass.log*pco2*day,random=~1|octo,
             correlation=corAR1(form=~day|octo),
              data=routine[routine$octo!="2-1"&routine$octo!="1-3"&routine$octo!="2-3",])
low.anova=Anova(low.lme,type="III")
low.lme.table=cbind(
  c("Mass","pCO~2~","Day"),
  round(low.anova$Chisq[2:4],2),
  low.anova$Df[2:4],
  round(low.anova$`Pr(>Chisq)`[2:4],5)
)
colnames(low.lme.table)=c("Factor","Chi-square", "DF", "p-value")

kable(low.lme.table)
```

So, the exclusion of those small octopuses does not make the effect pCO~2~ or day significantly related to RMR.
