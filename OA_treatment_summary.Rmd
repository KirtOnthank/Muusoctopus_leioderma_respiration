...
#  round(aggregate(pco2~treat,data=OA_Summary,FUN="sd")$pco2)),
  ...
  c(round(sqrt(mean(OA_Summary$ph.sd[OA_Summary$treat==1000]^2,na.rm=T)),3),
    round(sqrt(mean(OA_Summary$ph.sd[OA_Summary$treat==1800]^2,na.rm=T)),3))
  ),
  Alkalinity=paste0(round(aggregate(alk~treat,data=OA_Summary,FUN="mean")$alk),"±",
  c(round(sqrt(mean(OA_Summary$alk.sd[OA_Summary$treat==1000]^2,na.rm=T)),0),
    round(sqrt(mean(OA_Summary$alk.sd[OA_Summary$treat==1800]^2,na.rm=T)),0))
  ),
  Salinity=paste0(round(aggregate(salinity~treat,data=OA_Summary,FUN="mean")$salinity,1),"±",
  c(round(sqrt(mean(OA_Summary$salinity.sd[OA_Summary$treat==1000]^2,na.rm=T)),1),
    round(sqrt(mean(OA_Summary$salinity.sd[OA_Summary$treat==1800]^2,na.rm=T)),1))
  )
)
